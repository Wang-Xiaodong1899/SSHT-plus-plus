...
from loss import entropy, adentropy, MCC
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        # target label supvision loss
        Lx = F.cross_entropy(logits_x, targets_x, reduction='mean')

        
        loss = Lx
        # if 'Fix' in args.method:
        #     loss = Lx + args.lu*Lu
        optimizer.zero_grad()
        ...
        if args.method == 'S+T':
            ...
            ###
            loss = loss + args.lu * Lu
            loss.backward()
            optimizer_f.step()
            optimizer.step()
        elif args.method == 'ENT':
            loss_t = entropy(netC, feat_u_w, args.lamda)
            loss = loss + args.trade_off * loss_t
            loss.backward()
            optimizer_f.step()
            optimizer.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mcc_temp', default=2.5, type=float,help='temperature for mcc loss')
